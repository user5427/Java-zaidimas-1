...
public class EditorUI implements MouseUser{
	...
	public void update() {
		...
		updateFocusPoint();
		imageSlider.update();
		selectedTile = imageSlider.getTile();
		xSizeSlider.update();
		ySizeSlider.update();
		filesButton.update();
		menuButton.update();
	...
	private void displayWorld(Graphics g) {
		...
		for (int i = 0; i < levelData.getySize(); i++)
			for (int j = 0; j < levelData.getxSize(); j++) {
				...
				g.drawImage(tileImgs[levelData.getValueAt(index)], (int)((j * Game.TILES_SIZE + (int)focusPoint.x) * focusScale), (int)((i * Game.TILES_SIZE + (int)focusPoint.y) * focusScale), (int)(Game.TILES_SIZE * focusScale), (int)(Game.TILES_SIZE * focusScale), null);
			}
